...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === "DELETE" || (req.method === "POST" && req.body?.action === "delete")) {
      ...
      if (actualDriveFileId) {
        try {
          await deleteDriveFile(accessToken, actualDriveFileId);
        } catch (driveErr) {
          console.warn("[drive] Falha ao deletar do Drive (ignorando):", driveErr);
        }
      }

      const { error: dbError } = await supabase.from("case_files").delete().eq("id", caseFileId);
      if (dbError) throw dbError;

      if (stageId) {
        const { data: remaining } = await supabase.from("case_files").select("id").eq("stage_id", stageId).limit(1);
        if (!remaining || remaining.length === 0) {
          await supabase.from("case_stages").update({ status: "Fazer" }).eq("id", stageId);
        }
      }
    ...
    return res.status(500).json({
      error: "Falha na operação de Drive.",
